...
def dataset_generation(m,D_obs_prev,D_act_prev,T,policy_fn,policy_fn_est,policy_fn_idx,policy_fn_est_idx):
...
D_act_prev = act_data
T = 100
expert_policy = keras.models.load_model("PPOexpertMountainCar")
policy_list = [keras.models.load_model("PPOexpertMountainCar")]
policy_num=[1]
alpha = 0.1
...
with tf.Session() as sess:
    ...
    for i_smile in range(0,smile_iter):
        ...
        obsi_data,acti_data,save_mean,save_std = dataset_generation(m,D_obs_prev,D_act_prev,T,policy_curr,policy_prev,policy_curr_idx,policy_prev_idx)
...
print(Smile_results)
policy_exec_index_mountaincar = np.array(policy_exec)
num_rollouts_mean_array_mountaincar = np.array(num_rollouts_mean)
np.save('policy_exec_index.npy',policy_exec_index_mountaincar)
np.save('num_rollouts_mean_array.npy',num_rollouts_mean_array_mountaincar)
